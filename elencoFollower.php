<?php
    ...
    // Recupero l'utente di cui mostrare i follower
...
<head>
    ...
    <title>Elenco Follower - Asteria</title>
    ...
    <div class="main-container">
        ...
        <div class="lista-container">
            ...
            try {
                ...
                // Query con JOIN per prendere Foto e NomeUtente di chi segue
                $sql = "SELECT f.Seguente, u.Foto 
                        FROM follow f 
                        JOIN utenti u ON f.Seguente = u.NomeUtente 
                        WHERE f.Seguito = ?";
                ...
                if($preparata->rowCount() > 0) {
                    ...
                    foreach ($ris as $riga) {
                        $fotoFollower = !empty($riga['Foto']) ? $riga['Foto'] : 'Utente.png';
                        ?>
                        <div class="follower-card">
                            <div class="user-info">
                                <div class="avatar-container">
                                    <img src="<?php echo "UploadProfili/".$fotoFollower; ?>" alt="Profilo" class="follower-img">
                                </div>
                                <span class="username">@<?php echo htmlspecialchars($riga['Seguente']); ?></span>
                            </div>
                            <button class="btn-profilo" onclick="location.href='Profilo.php?user=<?php echo urlencode($riga['Seguente']); ?>'">
                                Vedi Profilo
                            </button>
                        </div>
                        <?php
                    ...
                    echo "<p class='empty-msg'>Nessun follower trovato per questo utente.</p>";
                }
                $connessione = null;
