...
function controllo(event) {
    ...
    let dataNascitaInput = document.getElementsByName('data')[0].value;
    ...
    let dataNascita = new Date(dataNascitaInput);
    let oggi = new Date();
    let eta = oggi.getFullYear() - dataNascita.getFullYear();
    let mese = oggi.getMonth() - dataNascita.getMonth();
    ...
    if (eta < 14) {
        alert("È necessario avere più di 14 anni per utiizzare Asteria");
        event.preventDefault();
        return false;
    }
    form.submit();
